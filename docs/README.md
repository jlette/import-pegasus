...
| **H5** | `Genre = 'Autre'` : quelle valeur PEGASUS ? | **Aucune : le traitement s'arrête.** `Genre` et `Sexe` sont deux données distinctes ; le sexe administratif est déterminé par le gestionnaire depuis le dossier de candidature, puis le fichier source est corrigé et relancé. Formalisée en **RG-02** (CDCF §5.7). Décision **provisoire**, à revoir si Phénix ouvre une valeur pour un genre non binaire | MOA, arbitrage du 17/08/2026 |
